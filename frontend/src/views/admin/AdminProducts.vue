    ...
    <table v-else class="data-table">
      ...
      <tbody>
        <tr v-for="product in products" :key="product.id">
          ...
          <td>
            <span class="status-tag" :class="product.active ? 'status-on' : 'status-off'">
...
const form = reactive<Product>({
  name: '', icon: '', price: 0,
  stock: 999, description: '', active: true,
})

const formStatus = computed({
  get: () => !!form.active,
  set: (v: boolean) => { form.active = v },
})
...
const openForm = (product?: Product) => {
  if (product) {
    ...
    Object.assign(form, { name: '', icon: '', price: 0, stock: 999, description: '', active: true })
  }
...
const handleSave = async () => {
  if (!form.name || !form.icon) {
    ElMessage.warning('请填写商品名称 and 图标')
    return
  }
  saving.value = true
  try {
